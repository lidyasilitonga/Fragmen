...
public class heroesData {
    ...
    private static int[] thumbnail = new int[]{R.drawable.ahmad_dahlan,R.drawable.ahmad_yani,
            R.drawable.bung_tomo,R.drawable.gatot_subroto,R.drawable.ki_hadjar_dewantara,R.drawable.mohammad_hatta,R.drawable.sudirman,R.drawable.sukarno,R.drawable.supomo,R.drawable.tan_malaka,R.drawable.hetlan};


    public static ArrayList<heroesModel> getListData(){
        heroesModel tModel = null;
        ArrayList<heroesModel> list = new ArrayList<>();
        for (int i = 0; i <title.length; i++){
            tModel = new heroesModel();
            tModel.setThumbnail(thumbnail[i]);
            tModel.setTitle(title[i]);
            tModel.setDetail(detail[i]);
            list.add(tModel);
        }
        return list;
    }
